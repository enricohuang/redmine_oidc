...
- **Automatic OIDC sign-in** -- optionally redirect users into the OIDC flow as soon as they open the Redmine login page
...
| **Auto-login with OIDC** | Automatically redirects users from `/login` into the OIDC flow. |
...
If you enable automatic OIDC sign-in, keep the emergency login URL available so administrators can still reach the local Redmine login form when the OIDC provider is unavailable:

```
https://your-redmine.example.com/login?local_login=1
```
